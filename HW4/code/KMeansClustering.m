...
%           point X(i, :) has been assigned to cluster c.
    ...
    while true        
        ...
        %                                                                     %
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        clusterDist = zeros(m, k);
        for clusterid = 1:k
            for xVec = 1:m
                
                clusterDist(xVec, clusterid) = sum((X(xVec,:) - centers(clusterid, :)).^2); 
            end
        end
        for i = 1:m
            [~, idx(i)] = min(clusterDist(i, :));
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %                                                                     %
        %                            END YOUR CODE                            %
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for i = 1:k
            numPointsinCluster = sum(idx == i);
            size(centers);
            i
            newVec = sum(X(idx == i, :)) ./ numPointsinCluster;
            centers(i, :) = newVec;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %                                                                     %
